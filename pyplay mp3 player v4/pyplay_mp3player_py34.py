...
#from tkinterdnd2 import * #for drag and drop feature
...
class Song:
    def __init__(self, filename, filepath, filesize):
        ...
        try:
            ...
            text = "Unable to load the MP3 File. \n\n" + self.filePath + "\n\n" + str(exp)
            self.Exception = exp
...
class Customize(Window):
    def __init__(self, parent):
        ...
        self.top.title(Window_Title)
        self.top.geometry("680x540+" + str(window.winfo_x()+100) + "+" + str(window.winfo_y()+100))
        self.top.attributes('-alpha', play_list.windowOpacity)
        columnOne = 10
        columnTwo = 250
        columnThree = 490
...
class Mp3TagModifierTool(Window):
    def __init__(self, fileIndex=0):
        ...
        if type(allButtonsFont) == StringVar:
            allButtonsFont = allButtonsFont.get()
        tk.Label(self.top, text="File Name:", fg=fontColor.get(), font=allButtonsFont, bg=color).place(x=columnOne, y=5)
        ...
        GrabAlbumYearButton = tk.Button(self.top, text="Search Album\Year Tags Online", command=self.grabAlbumAndYear, fg=fontColor.get(), font=allButtonsFont,
                                bg=color)
        ...
        ComposeFileNameButton = tk.Button(self.top, text="Set FileName from 'Artist - Title' Tags", command=self.composeFileName, fg=fontColor.get(), font=allButtonsFont,
                                        bg=color)
        ComposeFileNameButton.place(x=columnOne, y=175)
        ComposeArtistTitleButton = tk.Button(self.top, text="Set Artist/Title Tags from FileName", command=self.composeArtistTitle, fg=fontColor.get(), font=allButtonsFont,
                                        bg=color)
        ...
        self.undoMassRenameButton = tk.Button(self.top, text="Restore Previous FileNames to All Files.", command=self.restorePreviousNames, fg=fontColor.get(), font=allButtonsFont,
                                      bg=color)
        ...
        self.MassArtistTitleComposeButton = tk.Button(self.top, text="Set Artist/Title Tags from FileName to All Files", command=self.composeArtistTitleAll, fg=fontColor.get(), font=allButtonsFont,
                                      bg=color)
        if play_list.useMassFileEditor:
        ...
        self.undoMassArtistTitleComposeButton = tk.Button(self.top, text="Restore Previous Artist/Title Tags to All Files", command=self.undoComposeArtistTitleAll, fg=fontColor.get(), font=allButtonsFont,
                                      bg=color)
        ...
        GrabAlbumYearToAll = tk.Button(self.top, text="Search Album\Year Tags Online for All Files", command=self.grabAlbumYearToAllFiles, fg=fontColor.get(), font=allButtonsFont,
                              bg=color)
        if play_list.useMassFileEditor:
        ...
        undoAlbumYearToAll = tk.Button(self.top, text="Restore Previous Album/Year Tags to All Files", command=self.undoAlbumYearToAllFiles, fg=fontColor.get(), font=allButtonsFont,
                              bg=color)
...
class GrabLyricsTool(Window):
    def __init__(self, index="empty"):
        ...
        if self.songIndex != None: # make sure there is a song to search lyrics for.
            ...
            WindowDialog(window, "Playlist is empty.", Button1_Functionality=ButtonFunctionality("OK", None),
...
class ToolTip:
    def __init__(self, widget, text):
        self.widget = widget
        self.text = text
        noOfCharsOnLine = int((self.widget.winfo_reqwidth()+20) / calculateLetterWidthPixels())
        noOfLineNeeded = int(len(text) / noOfCharsOnLine)
        self.width = noOfCharsOnLine
        self.height = noOfLineNeeded
    ...
    def showTooltip(self):
        ...
        label = tk.Label(self.tooltipwindow, text=self.text, background="#ffffe0", relief='solid',
                         borderwidth=1, fg="black", width=self.width, height = self.height,
                         anchor="e", justify=tk.LEFT,
                         wraplength=self.widget.winfo_reqwidth())
        label.pack()
...
def load_file(fileToPlay=None): #this function is called when clicking on Open File Button.
    global play_list
    global listBox_Song_selected_index
    global window
        ...
        if isinstance(content, Playlist):
            ...
            if play_list.customElementBackground == None and play_list.customLabelBackground == None and play_list.customFont == None \
                ...
                if hasattr(menubar, "preferences_skin_menu"):
                    resetSkinMenuLabels()
                    if(customSkin==False):
                        menubar.preferences_skin_menu.entryconfigure(play_list.skin,
                                                                     label=(menubar.selectedToken + skinOptions[1][play_list.skin]))
                    else:
                        menubar.preferences_skin_menu.entryconfigure(menubar.preferences_skin_menu.index("end"),
                                         label=(menubar.selectedToken + skinOptions[1][menubar.preferences_skin_menu.index("end")]))
...
def load_directory(fpath=None): #this function is called when clicking on Open Directory Button.
    global play_list
    global listBox_Song_selected_index
    if fpath == None:
        play_list.dirFilePath = filedialog.askdirectory()
    else:
        play_list.dirFilePath = fpath
...
def play_music(): #this function is called when clicking on Play Button.
    """ play_music() function should always be called last.
        As this function triggers a scheduler which will only end during pause or stop
        statements placed ater play_music will not be executed until scheduler ends which produces huge delays in their execution
    """
    global play_list
    global visualSongNameLabel
    global s_rate
...
def pause_music(): #this function is called when clicking on Pause Button.
    ...
    if pygame.mixer.get_init(): #if the mixer is initialized it means music is loaded in the playback either currently playing, paused or stopped.
        try:
            if play_list.isSongPause == False and pygame.mixer.music.get_busy(): #Music is currently playing
...
def changingSkin(value: StringVar):
    SkinColor.set(value)
    changeSkin("<Double-Button>")
...
def changeSkin(event): #this function is called when clicking on Skin ComboBox
    ...
    if SkinColor.get() == "custom":
        ...
        if SkinColor.get() in skinOptions[1]: # if using predefined skins, with predefined backgrounds.
            ...
            if hasattr(menubar, "preferences_skin_menu"):
                resetSkinMenuLabels()
                menubar.preferences_skin_menu.entryconfigure(index, label=menubar.selectedToken + SkinColor.get())
...
def calculateScreenHeightWidth():
    ...
    screenHeight+= 100 if listbox["height"] == 20 else 120
...
def changePlaylistView():
    ...
    if play_list.viewModel == "COMPACT":
        if hasattr(menubar, "preferences_view_menu"):
            resetViewMenuLabels()
            menubar.preferences_view_menu.entryconfigure(0, label = (menubar.selectedToken + menubar.preferences_view_menu.entrycget(0, "label")))
        window.wm_attributes("-fullscreen", False)
        lineDistancePixels = PauseButton.winfo_reqwidth()*3 + play_list.buttonSpacing*5 + VolumeScale["width"]
        window.geometry(str(lineDistancePixels)+"x530+" + str(play_list.playerXPos) + "+" + str(play_list.playerYPos))# build a window
    ...
    if len(play_list.validFiles) > 0 and play_list.currentSongIndex:
        if pygame.mixer.get_init() != None:
            if pygame.mixer.music.get_busy() or play_list.isSongPause:
                ...
                updateRadioButtons()
                if play_list.isListOrdered==0: #if playlist is ordered by rating, update it in real time, since rating has changed
                    Song = play_list.validFiles[play_list.currentSongIndex]
                    play_list.validFiles.sort(key=lambda Song: Song.Rating, reverse=True)  # sort the list according to Rating
                    play_list.currentSongIndex=play_list.validFiles.index(Song)
                    displayElementsOnPlaylist()
...
def updateRadioButtons(): #this function is called when clicking on Song Rating - radio buttons.
    color = PauseButton["bg"]  # get the same color as every element
...
def changeFonts():
    ...
    ClearSearchButton["font"] = allButtonsFont
    # changing labels
    ...
    labelSearch["font"] = allButtonsFont
    # changing listbox
    listbox["font"] = allButtonsFont
    searchValue["font"] = allButtonsFont
...
def reSpacePositionElements(): #function called when changing skin, fonts or appearance to reposition the elements (buttons, listbox, labels, progressbar)
    ...
    # column 2
    labelSize.place(x=190+play_list.buttonSpacing, y=170)
    labelGenre.place(x=190+play_list.buttonSpacing, y=190)
    labelMonoStereoMode.place(x=190+play_list.buttonSpacing, y=210)
    labelFadeIn.place(x=190+play_list.buttonSpacing, y=230)
    labelFadeOut.place(x=190+play_list.buttonSpacing, y=250)
    labelFilesToPlay.place(x=190+play_list.buttonSpacing, y=270)
    labelNofPlays.place(x=190+play_list.buttonSpacing, y=290)
    labelWakeUp.place(x=190+play_list.buttonSpacing, y=310)
    ...
    labelPlaying["width"] = int(progressBarLength / calculateLetterWidthPixels())
...
def packPositionLabels(): #function called only at the start, to position the labels.
    ...
    # column 2
    labelSize.place(x=180, y=170)
    labelGenre.place(x=180, y=190)
    labelMonoStereoMode.place(x=180, y=210)
    labelFadeIn.place(x=180, y=230)
    labelFadeOut.place(x=180, y=250)
    labelFilesToPlay.place(x=180, y=270)
...
def pressedEnter(event): #this function will play next_song when ENTER is pressed
    if pygame.mixer.get_init():
        if pygame.mixer.music.get_busy():
            next_song()
        else:
...
def pressedShiftRight(event): #this function will play previous_song when SHIFT_RIGHT is pressed
    if pygame.mixer.get_init():
        if pygame.mixer.music.get_busy():
            previous_song()
        else:
...
def pressedKeyShortcut(event):
    if window.focus_get() != searchValue:
        if event.char == " ":
            pause_music()
...
def showAboutWindow():
    text = ("PyPlay MP3 Player is an application which was developed as challenge \n" +
...
def showPlaylistInfo():
    if(len(play_list.validFiles) > 0):
        ...
        WindowDialog(window, text, Button1_Functionality=ButtonFunctionality("OK", None), Button2_Functionality=ButtonFunctionality("Generate Playlist Report", exportPlaylistInfoToXls),
...
def packPositionListScrolOptionProgRadio(): #function called only at the start, to place the listbox, scrollbar, combobox, radiobuttons, progressbar,
    ...
    window.bind("<Control_R>", pressedCtrlRight)
    ...
    #window.bind("<KeyRelease>", showSearchResults)

    #searchValue.bind("<Shift_R>", playPreviousSearchItem)
...
def buttonAdjustments(): #this function will adjust some buttons (which are near labels) when the font gets changed
    ...
    VolumeScale["length"] = (PauseButton.winfo_reqwidth() - 34)  # make the volume scale have same width as the rest of the buttons , -7 because of the borders
    radioButtonLineHeight = 390
    labelSongRating.place(x=10, y=radioButtonLineHeight)
...
def playPreviousSearchItem():
    ...
    if play_list.danthologyMode == False:
        play_list.currentSongPosition = 0
    play_music() #this function should be called last
    # play_music() function should always be called last.
    # As this function triggers a scheduler which will only end during pause or stop
    # statements placed ater play_music will not be executed until scheduler ends which
    # produces huge delays in their execution
...
def createMenuBar():
    ...
    skin_menu.add_command(label = skinOptions[1][0], command=lambda: changingSkin(skinOptions[1][0]))
    skin_menu.add_command(label = skinOptions[1][1], command=lambda: changingSkin(skinOptions[1][1]))
    skin_menu.add_command(label = skinOptions[1][2], command=lambda: changingSkin(skinOptions[1][2]))
    skin_menu.add_command(label = skinOptions[1][3], command=lambda: changingSkin(skinOptions[1][3]))
    skin_menu.add_command(label = skinOptions[1][4], command=lambda: changingSkin(skinOptions[1][4]))
    skin_menu.add_command(label = skinOptions[1][5], command=lambda: changingSkin(skinOptions[1][5]))
    skin_menu.add_command(label = skinOptions[1][6], command=lambda: changingSkin(skinOptions[1][6]))
    skin_menu.add_command(label = skinOptions[1][7], command=lambda: changingSkin(skinOptions[1][7]))
    skin_menu.add_command(label = skinOptions[1][8], command=lambda: changingSkin(skinOptions[1][8]))
    skin_menu.add_command(label = skinOptions[1][9], command=lambda: changingSkin(skinOptions[1][9]))
    ...
    window.config(menu=menubar.root)
...
def DragAndDrop(event):
    ...
    if "} {" in data: #if multiple files were dropped
        ...
        data = tuple(data)
        load_file(data)
    if (".pypl" in data and os.path.isfile(data)):
        loadPlaylistFile(data)
    elif (".pypl" in data and os.path.isfile(data[1: len(data)-1])):
        loadPlaylistFile(data[1: len(data)-1])
    elif (".mp3" in data and os.path.isfile(data)):
        lst = []
        lst.append(data)
        load_file(lst)
    elif (".mp3" in data and os.path.isfile(data[1: len(data)-1])):
        lst = []
...
#window.dnd_bind('<<Drop>>', DragAndDrop)

Project_Title = "   PyPlay MP3 Player in Python     "
window.title(Project_Title)

window.geometry("300x510+" + str(play_list.playerXPos) + "+" + str(play_list.playerYPos))# build a window 500x430 pixels, at specified position
...
#Creating Volume:
VolumeScale = tk.Scale(window, from_=100, to=0, orient=tk.VERTICAL, fg=fontColor.get(), label="", length=(PauseButton.winfo_reqwidth() - 34) , sliderlength=10, width=7, showvalue=0,
...
labelPlayingFrame = tk.Frame(window, borderwidth=1, pady=1, padx=1, bg=SkinColor.get(), relief = tk.SUNKEN, height=60)
...
labelPlaying["wraplength"] = progressBarLength - 2 # - 4 becuase of the borders
labelPlaying["width"] = int(progressBarLength / calculateLetterWidthPixels())
labelPlayingFrame["width"] = progressBarLength
...
R1 = tk.Radiobutton(window,  variable=songRating, value=1, width=2, bg=radioButtonsDefaultColor, command=UpdateSongRating, fg = SkinColor.get(), selectcolor="black", font=allButtonsFont, borderwidth=0, relief=tk.GROOVE)
R2 = tk.Radiobutton(window,  variable=songRating, value=2, width=2, bg=radioButtonsDefaultColor, command=UpdateSongRating, fg = SkinColor.get(), selectcolor="black", font=allButtonsFont, borderwidth=0, relief=tk.GROOVE)
R3 = tk.Radiobutton(window,  variable=songRating, value=3, width=2, bg=radioButtonsDefaultColor, command=UpdateSongRating, fg = SkinColor.get(), selectcolor="black", font=allButtonsFont, borderwidth=0, relief=tk.GROOVE)
R4 = tk.Radiobutton(window,  variable=songRating, value=4, width=2, bg=radioButtonsDefaultColor, command=UpdateSongRating, fg = SkinColor.get(), selectcolor="black", font=allButtonsFont, borderwidth=0, relief=tk.GROOVE)
R5 = tk.Radiobutton(window,  variable=songRating, value=5, width=2, bg=radioButtonsDefaultColor, command=UpdateSongRating, fg = SkinColor.get(), selectcolor="black", font=allButtonsFont, borderwidth=0, relief=tk.GROOVE)
labelSongRating = tk.Label(window, text="Rating: ", compound=tk.LEFT, padx=3, pady=3, \
                        fg=SkinColor.get(), font=allButtonsFont, background = radioButtonsDefaultColor, borderwidth=0, relief=tk.GROOVE)

SearchForm = tk.Frame(window, borderwidth=1, pady=1, padx=1, bg=SkinColor.get(), relief = tk.SUNKEN)
...
labelSearch = tk.Label(SearchFrame, text="Search: ", padx=4, pady=0, anchor=tk.W, compound=tk.LEFT, width= 7 \
                        , fg=fontColor.get(), font=allButtonsFont, background = SkinColor.get(), borderwidth=0)
...
ClearSearchButton = tk.Button(SearchForm,  #the first parameter is the widget
                   ...
                   width= 6, command=SearchBoxClear, #the width of the button, and the function which get called when clicking it
                    bg=SkinColor.get(), fg=fontColor.get(),  #bg is the background color of the button, fg is the text color
                   font=allButtonsFont) #this is the font, size and type

searchValue = tk.Entry(SearchFrame, font=allButtonsFont, width=45, relief=tk.GROOVE,\
                     borderwidth=3, fg=fontColor.get(), background = SkinColor.get())
...
createMenuBar()
if hasattr(menubar, "preferences_skin_menu"):
    menubar.preferences_skin_menu.entryconfigure(play_list.skin, label = (menubar.selectedToken + skinOptions[1][play_list.skin]))
...
window.mainloop() #loop through the window
>>>>>>> 8fcd7ad7918d7fa32adbe11ee5fb17ebd426e2ee
